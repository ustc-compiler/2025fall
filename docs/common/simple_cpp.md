...
在之后的实验中，你可能会各种各样的段错误（Segmentation Fault），不要害怕！你可以通过 gdb/lldb 执行程序，使用 `bt` 命令查看崩溃时的调用栈。为了保留尽可能多的栈信息，你需要通过 `-g` 保留调试符号。

### 内存泄漏问题定位

[Address Sanitizer(ASan)](https://github.com/google/sanitizers/wiki/AddressSanitizer) 可以帮助我们定位内存泄漏问题。你通过在编译选项中指定 `-fsanitize=address` 启用 ASan。运行你的程序，如果存在内存泄漏，ASan会报告可能存在内存泄漏的分配点，如下所示：

```bash
$ clang -g -fsanitze=address main.c -o main # 使用ASan插桩
$ ./main # 如果存在内存泄漏会报告类似如下错误
==2802192==ERROR: LeakSanitizer: detected memory leaks

Direct leak of 40 byte(s) in 10 object(s) allocated from:
    #0 0x<addr> in malloc <file>:<line>  <---- 分配点
    #1 ...
